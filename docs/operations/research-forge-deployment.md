...
- **Broker State cleanup:** do not delete `/var/lib/research-forge/broker/state/<operation-hash>` while its PostgreSQL Operation is non-terminal. `cancelled.json` is broker-owned terminal recovery metadata: it is written before Docker stop/kill/remove and prevents a racing broker from persisting a success result. After terminal reconciliation and verified CAS registration, remove the entire operation directory through an audited maintenance procedure; a corrupt entry fails closed and requires operator investigation rather than a blind re-run.
