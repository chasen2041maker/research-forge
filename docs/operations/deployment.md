...
The runtime healthcheck verifies PostgreSQL, Redis, and the broker Unix socket. Redis contains versioned Stream envelopes rather than business state: baseline and repair Attempts are routed to distinct consumer-group streams, and the worker acknowledges the exact Stream receipt only after the complete Mission path has durably finished. Pending deliveries are reclaimed after `RF_REDIS_VISIBILITY_TIMEOUT_SECONDS`; after `RF_REDIS_MAX_DELIVERY_ATTEMPTS`, the queue atomically records a dead-letter envelope and acknowledges the exhausted pending receipt. PostgreSQL remains the source of truth. The reconciler scans stale `PREPARED` and `EXECUTING` Operations every 30 seconds and atomically emits one Outbox redelivery for the original Attempt; recovery reuses its original idempotency key. During a sandbox run the worker renews its owned Attempt lease every 10 seconds and stops that monitor before finalization, so the persisted optimistic version cannot race the evidence write. A repeated delivery after a crash is expected. Operation idempotency, CAS hashes, and the broker's atomically published checksummed result records are durable protections: after restart, the broker reloads a matching completion before it can invoke Docker again. Include `RF_BROKER_STATE_ROOT` in backups with the workspace and CAS roots.
