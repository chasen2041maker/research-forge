...
> Scope: this governance document is normative for `research_forge`. Research Studio is governed
> by its public-contract and code-boundary rules; it is not being rewritten to this architecture.
...
- 孤儿 Blob 通过 TTL GC；
- Git/CAS 已完成但 DB 未 Finalize 的完整发现与补记是 P0 恢复工作；当前 Reconciler
  只会重投递 stale Operation，不能宣称已修复该状态；
- DB Finalize 后 Queue 再投递必须返回同一结果。

完整协议见 [`ADR-0003`](../adr/0003-operation-ledger.md)。
